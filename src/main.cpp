...
int nCoinbaseMaturity = 19;
CBlockIndex* pindexGenesisBlock = NULL;
int nBestHeight = -1;

int64_t CharityAmount;
...
const string strMessageMagic = "BlueDragon Signed Message:\n";
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
        ...
        if(IsProofOfWork())
    {
            if (pindexBest->nHeight > 1){
                ...
                CharityAmount = 7 * COIN;
			}
        
		CBitcoinAddress address(!fTestNet ? CHARITY_ADDRESS : CHARITY_ADDRESS_TEST);
        CScript scriptPubKey;
        scriptPubKey.SetDestination(address.Get());
        if (vtx[0].vout[1].scriptPubKey != scriptPubKey)
            return error("ConnectBlock() : coinbase does not pay to the charity address)");
        if (vtx[0].vout[1].nValue < CharityAmount)
            return error("ConnectBlock() : coinbase does not pay enough to charity address");
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "BlueDragon", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "BlueDragon BDC Coin 2017";
        CTransaction txNew;
        txNew.nTime = 1488156993;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
...
//        txNew.vout[0].scriptPubKey = CHARITY_SCRIPT;
        ...
        block.nVersion = 1;
        block.nTime    = 1488156993;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 1098993;
		if(fTestNet)
        {
            block.nNonce   = 9250;
        }
       ...
       //// debug print
        assert(block.hashMerkleRoot == uint256("0xf1263a7c3c50bbe786e6b5e8f8d62e85c093976c3f21d9bdc239f3f52ba70ab9"));
        block.print();
